...
list_files_l1 = ['sim_logs/' + x for x in list_files_l1 if 'LOSLOG' in x]
...
ddf_conflog = dd.read_csv('sim_logs/*LOSLOG*.log', skiprows=9, header=None, names=header_columns, include_path_column=True)
...
df2 = pd.DataFrame(ddf_conflog.map_partitions(len).compute(), columns=['los'])
...
df3 = df2.pivot(index='rep', columns="density", values='los')
...
# only get files with CONFLOG in name
list_files_l1 = ['l1/' + x for x in list_files_l1 if 'LOSLOG' in x]

header_columns = ['exittime','starttime','timemindist','ACID1','ACID2','LAT1','LON1','ALT1','LAT2','LON2','ALT2','DIST']

# read all files and place in dataframe
ddf_conflog = dd.read_csv('l1/*LOSLOG*.log', skiprows=9, header=None, names=header_columns, include_path_column=True)
...
# reorganize the dataframe for plotting
df4 = df2.pivot(index='rep', columns="density", values='los')
...
fig.update_yaxes(title_text='LOS')
...
# set the y axis label as conflicts
fig1.update_yaxes(title_text='Conflicts')
fig1.update_xaxes(title_text='Density')
