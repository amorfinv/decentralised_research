...
def main():
    ...
    for sector in sector_array:
        ...
        # add to dictionary
        sector_length[str(sector)] = sector_flow

    # save node dictionary to JSON
    with open('airspace_design/sector_length.json', 'w') as fp:
        json.dump(sector_length, fp, indent=4)
    ...
    sector_dict = edge_sector.to_dict(orient='index')
    ...
    for key, value in sector_dict.items():
        ...
        if new_key in sector_dict_new.keys():
            sector_dict_new[new_key].append(new_value)
        else:
            sector_dict_new[new_key] = []
            sector_dict_new[new_key].append(new_value)
            
    # save edge dictionary as json
    with open('airspace_design/sectors.json', 'w') as fp:
        json.dump(sector_dict_new, fp, indent=4)
