...
# image directory
image_dir = '/Users/localadmin/Desktop/andresmorfin/decentralised_research/maps/images'

# get the style file
style_dir = '/Users/localadmin/Desktop/andresmorfin/decentralised_research/maps/styles'

# geotiff directory
geotiff_dir = '/Users/localadmin/Desktop/andresmorfin/decentralised_research/maps/geotif'

# list the contents (remove hidden files)
geotiff_files = os.listdir(geotiff_dir)

# remove anything that does not start with CONFLOG
# geotiff_files = [f for f in geotiff_files if f.startswith('CONFLOG')]

# now check inside images for the same files
...
for geo_tif in geotiff_files:
    ...
    pdf_file = f'{geo_tif[:-4]}.pdf'

    if pdf_file in image_files:
        print(f'{pdf_file} already exists')
        continue
    
    print(f'Attempting {geo_tif}')
    ...
    try:
        ...
        current_density = geo_tif.split('_')[1]
        if current_density == 'very':
            current_density = 'very_low'

        # logtype
        log_type = geo_tif.split('_')[0]
        ...
        print(f'Succesfully worked for {geo_tif}')

    except RuntimeError:
        print('---------------------')
        print(f'Failed for {geo_tif}')
        print('---------------------')
        break
